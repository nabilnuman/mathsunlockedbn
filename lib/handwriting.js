...
// A handful of classic digit/letter look-alikes ("2" vs "Z", "O" vs "0"…)
// that this ~86%-accurate model sometimes swaps. Never applied blind —
// only nudges a glyph from the letter to the digit when the digit was
// genuinely a close runner-up for THAT glyph specifically, and the rest
// of this ink reads as mostly digits (so a real answer that's actually
// meant to be a letter is left alone).
const CONFUSABLE_TO_DIGIT = { Z: "2", O: "0", S: "5", B: "8", G: "6", I: "1" };
...
function glyphsToString(glyphs, numberMode) {
  ...
  glyphs.forEach((g, i) => {
    ...
    else if (i > 0 && g.spaceBefore && !numberMode && !g.sup) out += " ";
    out += g.ch;
  });
  if (supOpen) out += ")";
  return out.trim();
...
export async function recognizeHandwriting(ink) {
  try {
    ...
    // never a letter — so those glyphs get the digit-only mask even when
    // the answer as a whole is free-form text. A bare "1" (a single thin
    // stroke) is otherwise genuinely ambiguous with "l"/"("/")"/"t", and
    // restricting the choices to digits is what actually resolves that.
    ...
    tagged.forEach((t, i) => {
      const restrict = numberMode || t.role === "num" || t.role === "den";
      maskData.set(restrict ? digitMask : openMask, i * HWR_CHARS.length);
    });
    ...
    // How this ink reads overall, ignoring confusable-pair glyphs for now —
    // the signal a nudge below is allowed to lean on.
    let digitCount = 0, letterCount = 0;
    const tops = probs.map((row) => {
      let best = 0;
      for (let k = 1; k < row.length; k++) if (row[k] > row[best]) best = k;
      if (NUM_IDX_SET.has(best)) digitCount++; else letterCount++;
      return best;
    });
    tagged.forEach((t, i) => {
      const row = probs[i];
      ...
      if (digitAlt) {
        const altIdx = HWR_CHARS.indexOf(digitAlt);
        // Was the digit a close runner-up for THIS glyph, and — excluding
        // this one — does the rest of the ink read as mostly digits?
        if (altIdx >= 0 && row[altIdx] >= row[top] * 0.35 && digitCount > letterCount - 1) top = altIdx;
      }
      t.g.ch = HWR_CHARS[top] ?? "";
    });

    const render = (role) => glyphsToString(tagged.filter((t) => t.role === role).map((t) => t.g), numberMode);
