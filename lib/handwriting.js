...
// "4", "5", "+" etc. together. Everything comes back tight (no spaces
// between glyphs) — the parser reads whitespace as nothing at all, so a
// gap here was only ever cosmetic, and a wide gap before a normal-sized
// character used to insert one anyway, which just as often broke a
// multi-digit number or a coefficient as it ever helped readability.
function segment(strokes) {
  const items = strokes
    .filter((s) => s && s.length)
    .map((s) => ({ s, b: strokeBox(s) }))
    .sort((a, b) => a.b.minX - b.b.minX);
  // A very small stroke — a decimal point, a comma, the dot on top of a
  // "j" — has to stay its OWN glyph: it must never be swallowed by a
  // neighbour, and nothing may merge into it. Otherwise the tiny-size
  // signal markTinyDots() relies on is gone and "1·5" comes back "155".
  const dimOf = (b) => Math.max(b.maxX - b.minX, b.maxY - b.minY);
  const dims = items.map((it) => dimOf(it.b)).filter((d) => d > 0).sort((a, b) => a - b);
  const medDim = dims.length ? dims[Math.floor(dims.length / 2)] : 0;
  const isTiny = (b) => { const d = dimOf(b); return d < 0.11 || (medDim > 0 && d < 0.33 * medDim); };
  const glyphs = [];
  for (const it of items) {
    ...
    if (merge) {
      ...
      glyphs.push({ strokes: [it.s], minX: it.b.minX, maxX: it.b.maxX, minY: it.b.minY, maxY: it.b.maxY, tiny: isTiny(it.b) });
    }
  }
  return glyphs;
}
...
// class indices allowed for a surd answer (digits + the root sign + . /) —
// the √ glyph is otherwise regularly misread as "E" / "5" / "r".
const SURD_IDX = HWR_CHARS.map((c, i) => (/[0-9]/.test(c) || c === "." || c === "/" || c === "√" ? i : -1)).filter((i) => i >= 0);
...
const CONFUSABLE_TO_DIGIT = { Z: "2", O: "0", D: "0", S: "5", B: "8", G: "6", b: "6", I: "1", L: "1", "(": "1", ")": "1", t: "1" };

// "t" is a trained class (EMNIST keeps it visually distinct from "T"),
// but no answer in this app ever actually needs a literal lowercase "t"
// — and its shape, a vertical stroke crossed by a horizontal one, is
// close enough to "+" that the model regularly swaps them (e.g. "x+3"
// read as "xt3"). Always read a "t" as "+" instead.
const T_IDX = HWR_CHARS.indexOf("t");
const PLUS_IDX = HWR_CHARS.indexOf("+");
...
function markSuperscripts(glyphs) {
  ...
  for (const g of glyphs) {
    ...
    // never a forced decimal point (markTinyDots ran first) — a real
    // raised digit keeps meaningful height, a dot does not
    g.sup = !g.tinyDot && h > 0.005 && h < typH * 0.62 && g.maxY < baseline - typH * 0.18;
  }
}

// A decimal point (or comma) is really defined by being TINY next to
// its neighbours — but rasterGlyph scales every glyph up to fill the
// same box regardless of how small it started, which erases exactly
// that signal, so the model ends up guessing its rounded shape against
// "0"/"8"/etc. purely on outline. A glyph much smaller than its
// neighbours (and not already a raised superscript) is forced to "."
// here instead of trusting that guess.
// Runs BEFORE markSuperscripts — a mid-line decimal point ("1·5", common
// in UK/Brunei schoolwork) sits high enough that the superscript test
// would otherwise claim it.
function markTinyDots(glyphs) {
  if (glyphs.length < 2) return;
  const sizeOf = (g) => Math.max(g.maxX - g.minX, g.maxY - g.minY);
  const sizes = glyphs.map(sizeOf).filter((s) => s > 0.002).sort((a, b) => a - b);
  if (!sizes.length) return;
  const typ = sizes[Math.floor(sizes.length / 2)];
  // vertical band of the full-size glyphs, to place a compact mark within it
  const bigs = glyphs.filter((g) => Math.max(g.maxX - g.minX, g.maxY - g.minY) >= typ * 0.6);
  const bandTop = bigs.length ? Math.min(...bigs.map((g) => g.minY)) : 0;
  const bandBot = bigs.length ? Math.max(...bigs.map((g) => g.maxY)) : 1;
  for (const g of glyphs) {
    const w = g.maxX - g.minX, h = g.maxY - g.minY, size = Math.max(w, h);
    // A decimal point / comma is: compact in BOTH axes (round-ish, unlike a
    // wide flat "-" / "="), small next to the digits, AND sitting on or
    // below the mid-line — a small glyph up in the top third is a power,
    // not a point, so leave it for markSuperscripts.
    const compact = w < 0.16 && h < 0.16;
    const mid = (g.minY + g.maxY) / 2;
    const band = bandBot - bandTop;
    const notRaised = mid >= bandTop + 0.4 * band;   // a small mark this low is a point, not a power
    const notTopZone = mid >= bandTop + 0.22 * band; // a single tap only fails this if it's clearly a superscript
    // A single-tap dot has a near-zero bounding box (one or two points) —
    // that on its own is the strongest "decimal point" signal there is, so
    // don't also require a size > 0 / relative-size test for it.
    const dab = size < 0.03;
    g.tinyDot = compact && ((dab && notTopZone) || (notRaised && size > 0 && (size < 0.12 || size < typ * 0.42)));
  }
}
...
// Turn a run of already-classified glyphs into text — always tight, no
// spaces between characters (see segment() above for why), with
// `^( … )` wrapped around a run flagged as a superscript.
...
export async function recognizeHandwriting(ink) {
  try {
    ...
    for (const grp of groups) {
      ...
      markTinyDots(gs);       // before markSuperscripts — see markTinyDots
      markSuperscripts(gs);
    ...
    const surdMask = new Float32Array(HWR_CHARS.length);
    SURD_IDX.forEach((i) => { surdMask[i] = 1; });
    const openMask = new Float32Array(HWR_CHARS.length).fill(1);
    const maskData = new Float32Array(tagged.length * HWR_CHARS.length);
    tagged.forEach((t, i) => {
      // A handwritten power in this app is always a number (or a leading
      // "-" / "/") — never a letter. Restrict raised glyphs to the digit
      // alphabet so a small open "4" isn't read as "V"/"y"/"u".
      const m = (t.g.sup || tightRole(t.role)) ? digitMask
        : surdMode ? surdMask
        : openMask;
      maskData.set(m, i * HWR_CHARS.length);
    });
    ...
    const info = probs.map((row, i) => {
      // Already decided by markTinyDots — don't let the model's raw (and
      // possibly letter-shaped) guess for a forced "." skew the vote for
      // everything else in the same answer.
      if (tagged[i].g.tinyDot) { certainDigits++; return { top: HWR_CHARS.indexOf("."), altIdx: -1 }; }
      let top = 0;
      for (let k = 1; k < row.length; k++) if (row[k] > row[top]) top = k;
      if (top === T_IDX) top = PLUS_IDX; // see T_IDX/PLUS_IDX above
    ...
    tagged.forEach((t, i) => {
      ...
      t.g.ch = t.g.tinyDot ? "." : HWR_CHARS[flipCandidates && altIdx >= 0 ? altIdx : top] ?? "";
    });

    // A minus/plus/equals/paren/comma is occasionally small enough and
    // sits high enough to pass the size/position test in
    // markSuperscripts (a short leading "-" before a number, say), but
    // it's only really an exponent when it's part of a genuinely raised
    // run — e.g. a negative exponent, "2^(-3)". So demote it back to a
    // normal character whenever the glyph right after it isn't also
    // raised, which is what actually tells the two cases apart.
    const OP_LIKE = new Set(["+", "-", "=", "(", ")", ","]);
    tagged.forEach((t, i) => {
      if (t.g.sup && OP_LIKE.has(t.g.ch)) {
        const next = tagged[i + 1];
        if (!next || next.role !== t.role || !next.g.sup) t.g.sup = false;
      }
    });

    const render = (role) => glyphsToString(tagged.filter((t) => t.role === role).map((t) => t.g));
