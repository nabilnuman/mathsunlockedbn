...
function markTinyDots(glyphs) {
  if (glyphs.length < 2) return;
  const sizes = glyphs.map((g) => Math.max(g.maxX - g.minX, g.maxY - g.minY)).filter((s) => s > 0.002);
  if (!sizes.length) return;
  const sorted = [...sizes].sort((a, b) => a - b);
  const typ = sorted[Math.floor(sorted.length / 2)];
  for (const g of glyphs) {
    const size = Math.max(g.maxX - g.minX, g.maxY - g.minY);
    g.tinyDot = !g.sup && size > 0 && size < typ * 0.22;
  }
}
