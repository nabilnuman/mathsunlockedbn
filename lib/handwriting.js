...
function glyphsToString(glyphs, numberMode) {
  ...
  glyphs.forEach((g, i) => {
    if (g.sup && !supOpen) { out += "^("; supOpen = true; }
      ...
      // A digit split across a gap is still one number ("16", never a
      // space-separated "1" and "6" — the parser reads spaces as nothing
      // at all, so that space would silently turn one number into two),
      // and a number pressed against a following letter is implicit
      // multiplication ("9x", never "9 x"). Both stay tight even when the
      // pen left extra room — e.g. for a superscript on the next glyph.
      const prevCh = glyphs[i - 1].ch;
      const staysTight = /[0-9]/.test(prevCh) && /[0-9A-Za-z]/.test(g.ch);
      if (!staysTight) out += " ";
    }
    out += g.ch;
  });
