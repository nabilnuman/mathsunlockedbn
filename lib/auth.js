...
export async function getLeaderboard(includeTeachers = false) {
  try {
    const { data, error } = await supabase.rpc(
      "get_leaderboard",
      includeTeachers ? { include_teachers: true } : {},
    );
    if (error) return [];
...
export async function getMyTeacher() {
  ...
  if (!me) return null;
  try {
    const { data, error } = await supabase.from("teachers").select("uid,name,admin").eq("uid", me).maybeSingle();
...
/* ---- teacher sign-up & licence activation ------------------------
   See supabase/schema.sql section 15. A teacher registers as a
   normal student account, then redeems an access code (the
   `teacher-activate` Edge Function) to gain the teacher role +
   a time-limited licence. Until then the account is student-only. */

// The caller's teacher-licence status. { active, expires_at?, note? }.
// Never throws — a missing/expired licence is { active: false }.
export async function myTeacherLicense() {
  try {
    const { data, error } = await supabase.rpc("my_teacher_license");
    if (error || !data) return { active: false };
    return data;
  } catch (e) { return { active: false }; }
}

// Register a teacher account: creates (or signs into) the name+PIN
// account exactly like a student, then records the sign-up as a lead.
// Returns { user, created }.
export async function registerTeacher({ name, pin, school, email }) {
  const res = await signInOrRegister(name, pin);
  try {
    const uid = res && res.user && res.user.id;
    if (uid) {
      await supabase.from("teacher_applicants").upsert(
        {
          uid,
          name: (name || "").trim(),
          school: school || null,
  ...
  } catch (e) { /* non-fatal — the applicants row is only a lead record */ }
  return res;
}

// Redeem an access code. -> { ok:true, expires_at, months } | { ok:false, error }
export async function activateTeacher(code) {
  let res;
  try {
    res = await supabase.functions.invoke("teacher-activate", { body: { code: (code || "").trim() } });
  } catch (e) {
...
// Admin: teacher sign-ups, newest first. [] on failure.
export async function recentTeacherApplicants(lim = 100) {
  try {
    const { data, error } = await supabase.rpc("recent_teacher_applicants", { lim });
    if (error) return [];
    return Array.isArray(data) ? data : [];
  } catch (e) { return []; }
}
...
export async function loadAssignments(classIds) {
  ...
  try {
    const { data, error } = await supabase
      .from("assignments")
      .select("id,class_id,topic_id,title,count,subs,due_at,created_at,archived")
...
export async function setAssignmentArchived(id, archived) {
  try { await supabase.from("assignments").update({ archived: !!archived }).eq("id", id); } catch (e) { /* ignore */ }
}

export async function createAssignment(classId, topicId, count, dueAt, title, subs) {
  try {
    const { data, error } = await supabase
      .from("assignments")
      .insert({ class_id: classId, topic_id: topicId, count, due_at: dueAt || null, title: title || null, subs: subs || [] })
      .select()
      .maybeSingle();
    if (error) return { error: error.message };
...
/* Beta feedback. context = { screen, topic, question, version }. */
export async function sendFeedback({ name, message, rating, context }) {
  try {
...
/* Web-push subscription for this device. RLS scopes rows to auth.uid(). */
...
export async function deletePushSubscription(endpoint) {
  try { await supabase.from("push_subscriptions").delete().eq("endpoint", endpoint); } catch (e) { /* ignore */ }
  return { ok: true };
}

/* Ask the send-push Edge Function to notify the other side of an event.
   kind: "blitz" (id = blitz_challenges.id) | "homework" (id = assignments.id,
   caller = the teacher, notifies the class) | "hwdone" (id = assignments.id,
   caller = the student who just finished it for the first time, notifies
   the class's teacher).
   Safe to call always — it's a no-op if push isn't configured server-side. */
export async function notifyPush(kind, id) {
  try {
    await supabase.functions.invoke("send-push", { body: { kind, id } });
  } catch (e) { /* best-effort */ }
}

/* Fetch (creating one if it doesn't exist yet) a student's parent-link
   token, on a teacher's or admin's behalf. Server-side authorized: admin
   for any student, a teacher only for a student in one of their own
   classes. Throws on failure (caller shows the message). */
export async function getParentLinkFor(uid) {
  const { data, error } = await supabase.rpc("get_parent_link", { target_uid: uid });
  if (error) throw new Error(error.message || "Couldn't get that student's parent link.");
  return data;
}

/* Admin roster: every registered student's profile + last-active time.
   Returns [] for non-admin callers (RLS inside the function). */
...
// Admin: the three engagement numbers worth checking on (retention,
// progression, comeback-after-a-bad-session). { error } on failure (the
// caller isn't an admin, the SQL function hasn't been run yet, etc.) so
// the UI can show WHY instead of a bare "couldn't load."
export async function adminEngagementMetrics() {
  try {
    const { data, error } = await supabase.rpc("get_engagement_metrics");
    if (error) return { error: error.message || String(error) };
    if (data == null) return { error: "No data returned — are you signed in as an admin?" };
    return data;
  } catch (e) { return { error: (e && e.message) || String(e) }; }
}

// Admin: day-by-day count of distinct students who earned XP that day,
// for the "players right now" style chart on Admin -> Metrics. Covers
// only the last ~35 days (profile.dayHistory's rolling window) — never
// throws; { error } on failure so the UI can show why instead of a bare
// "couldn't load."
export async function adminDailyActive() {
  try {
    const { data, error } = await supabase.rpc("get_daily_active_students");
    if (error) return { error: error.message || String(error) };
    if (data == null) return { error: "No data returned — are you signed in as an admin?" };
    return { days: data };
  } catch (e) { return { error: (e && e.message) || String(e) }; }
...
// How many real students hold each achievement, out of how many such
// profiles exist — { total, counts: { [achievementId]: count } }. Open to
// any signed-in user (it's an aggregate, no per-student detail), so the
// Achievements panel can show "X% of players have this" to everyone, not
// just admins. { error } on failure.
export async function achievementStats() {
  try {
    const { data, error } = await supabase.rpc("get_achievement_stats");
    if (error) return { error: error.message || String(error) };
    if (data == null) return { error: "No data returned." };
    return data;
  } catch (e) { return { error: (e && e.message) || String(e) }; }
}

// Admin: every teacher account (activated + pending sign-ups). [] on failure.
export async function adminTeachers() {
  try {
    const { data, error } = await supabase.rpc("admin_teachers");
    return error || !Array.isArray(data) ? [] : data;
  } catch (e) { return []; }
}

/* Daily Challenge. One row per player per Brunei day; time is final. */
export async function submitDailyResult(seconds, name) {
  try {
    const { error } = await supabase.from("daily_results").insert({
      seconds: Math.round(Number(seconds) * 100) / 100,
      name: name || null,
    });
    // a duplicate (already played today) is not a real failure
    return { ok: !error || error.code === "23505", error: error && error.code !== "23505" ? error.message : null };
  } catch (e) {
    return { ok: false, error: String(e && e.message || e) };
  }
}
/* A day's board, fastest first, teachers excluded. [{ uid, name, seconds }].
   `day` is an optional "YYYY-MM-DD" (Brunei); omit for today. */
export async function dailyBoard(day) {
  try {
    const { data, error } = await supabase.rpc("daily_board", day ? { d: day } : {});
    return error || !Array.isArray(data) ? [] : data;
  } catch (e) { return []; }
}
/* The caller's own time for today, or null if they haven't played. */
export async function myDailyResult() {
  try {
    const { data, error } = await supabase.rpc("my_daily");
    if (error || data == null) return null;
    return Number(data);
  } catch (e) { return null; }
}

/* Daily Slide (sliding-tile puzzle). One row per player per Brunei day,
   same shape as the Daily Challenge above — time is final. */
export async function submitSlideResult(seconds, name) {
  try {
    const { error } = await supabase.from("slide_results").insert({
      seconds: Math.round(Number(seconds) * 100) / 100,
      name: name || null,
    });
    return { ok: !error || error.code === "23505", error: error && error.code !== "23505" ? error.message : null };
...
/* A day's Daily Slide board, fastest first. [{ uid, name, seconds }].
   `day` is an optional "YYYY-MM-DD" (Brunei); omit for today. */
export async function slideBoard(day) {
  try {
    const { data, error } = await supabase.rpc("slide_board", day ? { d: day } : {});
    return error || !Array.isArray(data) ? [] : data;
  } catch (e) { return []; }
}
/* The caller's own Daily Slide time for today, or null if not played. */
export async function mySlideResult() {
  try {
    const { data, error } = await supabase.rpc("my_slide");
    if (error || data == null) return null;
    return Number(data);
  } catch (e) { return null; }
}

/* Teacher-only inbox; returns [] for everyone else. */
export async function recentFeedback(lim = 200) {
  try {
    const { data, error } = await supabase.rpc("recent_feedback", { lim });
